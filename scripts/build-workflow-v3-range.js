...
const system = [
  ...
  '}',
  '',
  'Regras: 3 a 5 indicadores, 2 a 5 alertas, 2 a 5 recomendacoes, 3 a 5 termos no glossario (apenas os realmente usados na analise). Cite clientes concretos do top_devedores quando fizer sentido. Se faltar sinal, devolva array vazio.'
].join('\\n');
...
const analise = (p && typeof p === 'object') ? {
  ...
  indicadores: [], alertas: [], recomendacoes: [], glossario: [],
...
const system = [
  ...
  'Responda APENAS com um objeto JSON valido (sem markdown, sem code fences, sem texto fora do JSON).',
  'Tom: de especialista que orienta — claro, didatico e objetivo, portugues BR, foco em acao operacional de producao/reposicao. Nenhum jargao sem explicacao.',
  '',
  'Schema obrigatorio:',
  '{',
  ...
  '  "recomendacoes": [ { "prioridade": "alta|media|baixa", "acao": "repor|produzir|promover|reduzir_compra|investigar", "produto": "string ou diversos", "motivo": "string", "fundamentacao": "por que esta acao, ancorada em um conceito de PCP (ex.: cobertura abaixo do lead time gera ruptura)", "impacto_esperado": "string" } ],',
  '  "glossario": [ { "termo": "string", "definicao": "explicacao curta e simples do termo usado na analise" } ]',
  '}',
  '',
  'Regras: 3 a 5 indicadores, ate 10 itens em reposicao_urgente (os de menor cobertura), 2 a 6 alertas, 2 a 5 recomendacoes, 3 a 5 termos no glossario (apenas os realmente usados). Cite produtos concretos. Se faltar sinal, devolva array vazio.'
].join('\\n');
...
const analise = (p && typeof p === 'object') ? {
  ...
  indicadores: [], reposicao_urgente: [], alertas: [], recomendacoes: [], glossario: [],
  erro: 'Falha ao interpretar resposta do agente',
  raw: String(raw).slice(0, 400)
};
